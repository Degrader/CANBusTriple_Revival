...
{
public:
    explicit SerialCommand( MessageQueue *q );
    void tick() override;
...
private:
    ...
    bool btMessageAllowed(unsigned int busId, unsigned short frameId);
    unsigned short btMessageIdFilters[4][2];
...
SerialCommand::SerialCommand( MessageQueue *q )
{
    ...
    lastBluetoothRX = 0;
    byteCount = 0;
    memset( btMessageIdFilters, 0, sizeof(btMessageIdFilters) ); // No BT filter set = forward everything
}
...
void SerialCommand::printMessageToSerial( Message msg )
{
    if ( activeSerial == &Serial1 ) {
        ...
        if ( btRateLimit() ) return;
        // Bluetooth per-bus message ID filter
        if ( !btMessageAllowed(msg.busId, msg.frame_id) ) return;
    }
...
// Should this message be forwarded over the Bluetooth link?
// A bus with no filter set (both IDs 0) forwards everything, matching
// the "0x04 <bus> 0x0000 0x0000" documented "disable filter" command.
bool SerialCommand::btMessageAllowed(unsigned int busId, unsigned short frameId)
{
    if ( busId >= 4 ) return true;

    unsigned short filter1 = btMessageIdFilters[busId][0];
    unsigned short filter2 = btMessageIdFilters[busId][1];

    if ( filter1 == 0 && filter2 == 0 ) return true;

    return frameId == filter1 || frameId == filter2;
}
...
void SerialCommand::getAndSaveEeprom()
{
    ...
    byte* settings = reinterpret_cast<byte *>(&cbt_settings);
